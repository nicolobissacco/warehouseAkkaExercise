...
class SupplierAdapter extends EventAdapter {
  ...
  override def fromJournal(event: Any, manifest: String): EventSeq = event match {
    ...
    //case event: Supplier.AddedProduct => EventSeq(event)
    //case event: Supplier.RemovedProduct => EventSeq(event)
  }

  override def toJournal(event: Any): Any = event match {
    case event: Supplier.Created => Tagged(event, tag)
    //case event: Supplier.AddedProduct => Tagged(event, tag)
    //case event: Supplier.RemovedProduct => Tagged(event, tag)
  }
}
