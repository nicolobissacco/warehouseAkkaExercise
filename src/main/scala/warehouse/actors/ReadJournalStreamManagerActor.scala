...
import warehouse.actors.projector.{AckMessage, CompleteMessage, InitMessage, RestartStream}
import akka.NotUsed
...
trait ReadJournalStreamManagerActor[T] extends DiagnosticActorLogging {
  ...
  protected def startStream(offset: Offset): Unit = {
    println(s"Consumer start stream on Cassandra journal: ${self.path.name}")
    ...
    println(s"Journal reader started on Cassandra journal - ${self.path.name}")
  }
  ...
  protected def manageJournalStream(offset: Offset): Receive = {
    ...
    case InitMessage =>
      sender ! AckMessage
      println(s"Consumer initialize stream on Cassandra journal - ${self.path.name}")
    case Status.Failure(cause: Throwable) =>
      ...
      println(cause, s"Consumer on Cassandra journal received failure message!")
    case CompleteMessage =>
      println(s"Consumer on Cassandra journal received complete message!")
      context.system.scheduler.scheduleOnce(30.seconds, self, RestartStream)(context.system.dispatcher)
    case AckMessage =>
      println(s"Consumer on Cassandra journal received AckMessage message while in consuming: message ignored!")
    case other =>
      println(s"Consumer on Cassandra journal received unknown message: $other")
  }

  /** Complete the stream */
  override def preRestart(reason: Throwable, message: Option[Any]): Unit = {
    println(reason, s"Actor restarted due to unhandled exception when processing $message")
